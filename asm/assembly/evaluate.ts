...
class Weight {
    readonly nI: i32 = 256;
    readonly nH1: i32 = 256;
    readonly nH2: i32 = 128;
    readonly nH3: i32 = 64;
    readonly nH4: i32 = 32;
    readonly nH5: i32 = 32;
...
class Perceptron {
    ...
    run(_board1: BitBoard, _board2: BitBoard, cb: u64): f64 {
    ...
    backPropagation(_board1: BitBoard, _board2: BitBoard, cb: u64, t: number, eta: number): void {
        const count = _board1.count() + _board2.count();
        const weight = this.weight[Math.floor(count / 13)];
        //const weight = this.weight;
...
export class Eval {
    ...
    read(list: f64[]): void {
        ...
        for (let i = 0; i < 65; i++) {
            ...
            this.cb.push(new Array());
            for (let j = 0; j < 64; j++) {
                this.board1[i].push(list[i * 256 + j]);
            }
            for (let j = 0; j < 64; j++) {
                this.board2[i].push(list[i * 256 + j + 64]);
            }
            for (let j = 0; j < 64; j++) {
                this.space[i].push(list[i * 256 + j + 128]);
            }
            for (let j = 0; j < 64; j++) {
                this.cb[i].push(list[i * 256 + j + 192]);
            }
        }
    ...
    @inline
    _eval(_board1: BitBoard, _board2: BitBoard, _cb: BitBoard): f64 {
        let r: f64 = 0;
        let n: u32 = _board1.count() + _board2.count();
        let board1: u64 = _board1.board, board2: u64 = _board2.board;
        let cb: u64 = _cb.board;
    ...
    learn(_board1: BitBoard, _board2: BitBoard, _cb: BitBoard, t: number, eta: number): void {
    ...
    @inline
    evaluate(board: Board, color: number): f64 {
    ...
    // runPerceptron(board: Board, color: number): f64 {
    //     let board1: BitBoard, board2: BitBoard;
    ...
    // writeP(path: string): void {
    //     this.perceptron.write(path);
    // }

    // readP(path: string): void {
